export type StoryEvent = {
  id: string;
  date?: string;
  title: string;
  text: string[];
  quote?: string;
  image?: string;
  imageAlt?: string;
  video?: string;
  mediaCaption?: string;
  mapUrl?: string | null;
  layout: 'map' | 'editorial' | 'fullscreen' | 'letter' | 'type';
};

export type GalleryImage = {
  src: string;
  alt: string;
  rotate: number;
  position?: string;
};

import { siteAsset } from '@/src/lib/site-asset';

export const story = {
  access: {
    code: '12.05.2025',
    prompt: 'Введи дату нашего знакомства',
    hint: 'дд.мм.гггг',
    error: 'Неправильно',
    button: 'Открыть',
  },
  people: {
    first: 'Костя',
    second: 'Ксюша',
    pair: 'КОСТЯ & КСЮША',
    nicknameForKostya: 'котей',
  },
  dates: {
    met: '2025-05-12T20:00:00+05:00',
    firstMeeting: '2025-05-27T19:00:00+05:00',
    relationship: '2025-10-22T00:00:00+05:00',
    ksyushaBirthday: '2025-09-30T00:00:00+05:00',
    kostyaBirthday: '2026-05-03T00:00:00+05:00',
    finalBell: '2026-05-26T00:00:00+05:00',
  },
  hero: {
    since: '12.05.2025',
    kicker: 'Наша история',
    button: 'Начать',
    lead: 'С одного лайка — до всего, что стало нашим.',
  },
  intro: {
    date: '12 МАЯ 2025',
    title: 'Всё началось с одного лайка.',
    text: 'Я увидел твоё видео, подумал, что ты очень красивая и милая, и нажал на сердечко. Вечером ты ответила взаимно. Мы сразу словили общий вайб. Ты понравилась мне настолько, что я уже тогда выстраивал план и почему-то знал: ты будешь моей.',
    video: siteAsset('/videos/daivinchik.mp4'),
    videoAlt: 'Видео Ксюши из анкеты, с которого началась история',
    videoLabel: 'Первый кадр',
    videoDuration: '14 сек',
    after: 'А на следующий день ты спросила: «А как тебя зовут вообще?»',
  },
  events: [
    {
      id: 'first-meeting',
      date: '27 МАЯ 2025',
      title: '«Ты Костя?»',
      text: [
        'Ты гуляла с подругами, а я предложил проводить тебя вечером до дома. Мы почти случайно встретились: я подошёл, и первым делом ты спросила — «Ты Костя?»',
        'Мы немного погуляли. Я проводил тебя, но не до самого подъезда — тогда ты ещё переживала, что родители узнают обо мне. На прощание ты меня обняла.',
      ],
      quote: '«Нормально ли, что я обняла тебя?»',
      image: siteAsset('/images/story/first-meeting-map.png'),
      imageAlt: 'Место первой встречи Кости и Ксюши на карте',
      mediaCaption: 'Место, где всё стало настоящим',
      mapUrl: 'https://maps.app.goo.gl/J4i54wBFp2TcJkhx8',
      layout: 'map',
    },
    {
      id: 'birthday',
      date: '30 СЕНТЯБРЯ 2025',
      title: 'Твой день рождения.',
      text: [
        'Я пришёл поздравлять тебя вместе с твоими подругами.',
        'Подарил букет, ебнул тебе тортом в лицо и сразу смотался. А волновался в тот момент — пиздец.',
      ],
      video: siteAsset('/videos/her-birthday.mp4'),
      layout: 'type',
    },
    {
      id: 'october',
      date: 'ОКТЯБРЬ 2025',
      title: 'Начало.',
      text: [
        'Мы ещё не встречались, но всё уже вело к этому. Сначала я провожал тебя до дома, нёс твою сумку, впервые приобнял и шёл с тобой под руку.',
        'Потом была лавочка в городке. Я с огромным страхом положил руку тебе на плечо, а ты — голову мне на грудь. Я что-то ляпнул, а ты убрала голову.',
        'На следующей прогулке я просто взял твою руку, положил её в свою и пошёл дальше как ни в чём не бывало.',
      ],
      quote: '«Мне кажется, ты ахуел»',
      image: siteAsset('/images/story/october-walk.jpg'),
      imageAlt: 'Костя и Ксюша идут вечером, снятые со спины другом Тиграном',
      layout: 'fullscreen',
    },
    {
      id: 'official',
      date: '22 ОКТЯБРЯ 2025',
      title: 'Мы решили быть вместе.',
      text: [
        'Через неделю мы спокойно гуляли за руку. А на прощание я решил впервые поцеловать тебя в лоб.',
      ],
      quote:
        'Но ты перехватила инициативу, поцеловала меня в губы, обняла и ушла. А я в полном ахуе шёл домой.',
      layout: 'type',
    },
    {
      id: 'disco',
      date: '31 ОКТЯБРЯ 2025',
      title: 'Я пошёл туда только ради тебя.',
      text: [
        'Прошло совсем немного времени с начала отношений. На той дискотеке был наш первый медленный танец.',
      ],
      image: siteAsset('/images/story/slow-dance.jpg'),
      imageAlt: 'Костя и Ксюша танцуют медленный танец на дискотеке',
      layout: 'fullscreen',
    },
    {
      id: 'kostya-birthday',
      date: '3 МАЯ 2026',
      title: 'Ты пришла ровно в 00:00.',
      text: [
        'Ты подарила мне LEGO, и я на мгновение опять почувствовал себя ребёнком. Ещё — коробочку с нашими фотографиями, сладостями и письмом от руки.',
        'Письмо растрогало меня сильнее всего. Его я хочу сохранить здесь целиком — таким, каким ты его сделала.',
      ],
      image: siteAsset('/images/story/kostya-birthday.jpg'),
      imageAlt: 'Ксюша целует Костю, испачканного тортом, в его день рождения',
      layout: 'letter',
    },
    {
      id: 'final-bell',
      date: '26 МАЯ 2026',
      title: 'Твой последний звонок.',
      text: [
        'Год назад мы только начинали узнавать друг друга. А теперь я стою рядом с тобой на твоём последнем звонке и понимаю, сколько всего успело произойти за этот год.',
      ],
      image: siteAsset('/images/story/final-bell.jpg'),
      imageAlt:
        'Костя и Ксюша целуются на последнем звонке Ксюши, в руках большой букет',
      layout: 'fullscreen',
    },
  ] satisfies StoryEvent[],
  gallery: [
    {
      src: siteAsset('/images/gallery/01-balaclavas.jpg'),
      alt: 'Костя и Ксюша в балаклавах',
      rotate: -2,
    },
    {
      src: siteAsset('/images/gallery/02-close-up.jpg'),
      alt: 'Близкое совместное фото Кости и Ксюши',
      rotate: 0,
    },
    {
      src: siteAsset('/images/gallery/03-mirror.jpg'),
      alt: 'Совместное фото Кости и Ксюши в зеркале',
      rotate: 1.5,
    },
    {
      src: siteAsset('/images/gallery/04-cheek-kiss.jpg'),
      alt: 'Костя целует Ксюшу в щёку',
      rotate: -1,
    },
    {
      src: siteAsset('/images/gallery/05-cafe.jpg'),
      alt: 'Смешное совместное селфи в кафе',
      rotate: 2,
    },
    {
      src: siteAsset('/images/gallery/06-sun.jpg'),
      alt: 'Совместное селфи в солнечный день',
      rotate: -1.5,
    },
    {
      src: siteAsset('/images/gallery/08-park.jpg'),
      alt: 'Смешное фото Кости и Ксюши в парке',
      rotate: 1,
    },
    {
      src: siteAsset('/images/gallery/09-night-kiss.jpg'),
      alt: 'Костя целует Ксюшу в щёку вечером',
      rotate: -2,
    },
    {
      src: siteAsset('/images/gallery/10-night.jpg'),
      alt: 'Смешное совместное вечернее селфи',
      rotate: 1.5,
    },
    {
      src: siteAsset('/images/gallery/11-mirror.jpg'),
      alt: 'Костя и Ксюша вместе перед зеркалом',
      rotate: -0.5,
    },
  ] satisfies GalleryImage[],
  summer: {
    title: 'Мы просто становились ближе.',
    text: 'Мы постоянно переписывались, всё чаще гуляли и понемногу переставали стесняться друг друга. Тогда ещё ничего не было, но мне уже очень нравилось просто быть рядом с тобой.',
    galleryLabel: 'Первые обычные совместные фотографии',
  },
  everyday: {
    title: 'Сближение.',
    text: 'Мы старались давать друг другу больше любви. Всё ещё стеснялись — но уже знали, что мы вместе.',
    video: siteAsset('/videos/slide-8.mp4'),
    galleryLabel: 'Совместные фотографии Кости и Ксюши',
  },
  soundtrack: {
    intro: {
      title: 'Любовь HD1080',
      artist: 'Каспийский Груз',
      file: siteAsset('/music/track-1.mp3') as string | null,
    },
  },
  letter: {
    image: siteAsset('/images/story/birthday-letter.jpg'),
    alt: 'Рукописное письмо Ксюши Косте на день рождения',
    quote: '«Люблю тебя, Костенька»',
  },
  counter: {
    eyebrow: 'С 22 октября 2025',
    title: 'Мы вместе уже',
  },
  final: {
    eyebrow: 'Сегодня',
    title: 'Ксюша, я очень тебя люблю.',
    paragraphs: [
      'Мне нравится твоя улыбка. Твои карие глаза — и то, как, когда ты плачешь, в них появляется немного зелёного. Мне нравится, как ты ухаживаешь за собой, как смеёшься и как иногда мы оба можем вести себя как два ребёнка.',
      'Я люблю наши прогулки, наши тупые шутки, объятия, поцелуи и даже те моменты, когда мы просто молчим рядом. Я уже не представляю свою жизнь без тебя в ней.',
      'Спасибо тебе за тот лайк 12 мая. За первую прогулку. За тот первый поцелуй. За твоё письмо. За все наши фотографии и воспоминания. И просто за то, что ты есть у меня.',
    ],
    signature: 'С днём рождения, Ксюша. Я тебя люблю. 🤍',
    author: '— Костя',
    image: siteAsset('/images/story/final-slide.jpg'),
    imageAlt: 'Костя и Ксюша вместе',
  },
  footer: {
    timeline: '12.05.2025 — ∞',
    note: 'Сделано Костей для Ксюши',
  },
} as const;
